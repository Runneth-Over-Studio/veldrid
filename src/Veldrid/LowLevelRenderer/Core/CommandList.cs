...
public abstract class CommandList : DeviceResource, IDisposable
{
    ...
    public void SetGraphicsResourceSet(uint slot, ResourceSet rs, uint dynamicOffsetsCount, ref uint dynamicOffsets)
    {
        ...
        {
            throw new Exception($"A graphics Pipeline must be active before {nameof(SetGraphicsResourceSet)} can be called.");
        }
        ...
        {
            throw new Exception($"Failed to bind ResourceSet to slot {slot}. The number of resources in the ResourceSet ({setLength}) does not match the number expected by the active Pipeline ({pipelineLength}).");
        }
    ...
    public unsafe void SetComputeResourceSet(uint slot, ResourceSet rs, uint dynamicOffsetsCount, ref uint dynamicOffsets)
    {
        ...
        {
            throw new Exception($"A compute Pipeline must be active before {nameof(SetComputeResourceSet)} can be called.");
        }
    ...
    public void ClearColorTarget(uint index, RgbaFloat clearColor)
    {
        ...
        {
            throw new Exception($"Cannot use ClearColorTarget. There is no Framebuffer bound.");
        }
    ...
    public void ClearDepthStencil(float depth, byte stencil)
    {
        ...
        if (_framebuffer == null)
        {
            throw new Exception($"Cannot use ClearDepthStencil. There is no Framebuffer bound.");
        }
    ...
    public void DrawIndexed(uint indexCount, uint instanceCount, uint indexStart, int vertexOffset, uint instanceStart)
    {
        ...
        {
            throw new Exception("Drawing with a non-zero base vertex is not supported on this device.");
        }
        if (!_features.DrawBaseInstance && instanceStart != 0)
        {
            throw new Exception("Drawing with a non-zero base instance is not supported on this device.");
        }
#endif
    ...
    private static void ValidateIndirectOffset(uint offset)
    {
        if ((offset % 4) != 0)
        {
            throw new Exception($"{nameof(offset)} must be a multiple of 4.");
        }
    }
    ...
    private void ValidateDrawIndirectSupport()
    {
        ...
        {
            throw new Exception($"Indirect drawing is not supported by this device.");
        }
    }
    ...
    public void CopyTexture(Texture source, Texture destination)
    {
        ...
        {
            throw new Exception("Source and destination Textures are not compatible to be copied.");
        }
#endif
        ...
        uint layerCount)
    {
        ...
        {
            throw new Exception($"The given copy region is empty.");
        }
        if (layerCount == 0)
        {
            throw new Exception($"{nameof(layerCount)} must be greater than 0.");
        }
        ...
        {
            throw new Exception($"The given copy region is not valid for the source Texture.");
        }
        ...
        {
            throw new Exception($"The given copy region is not valid for the destination Texture.");
        }
        if (srcMipLevel >= source.MipLevels)
        {
            throw new Exception($"{nameof(srcMipLevel)} must be less than the number of mip levels in the source Texture.");
        }
        uint effectiveSrcArrayLayers = (source.Usage & TextureUsage.Cubemap) != 0
            ? source.ArrayLayers * 6
            : source.ArrayLayers;
        ...
        {
            throw new Exception($"An invalid mip range was given for the source Texture.");
        }
        if (dstMipLevel >= destination.MipLevels)
        {
            throw new Exception($"{nameof(dstMipLevel)} must be less than the number of mip levels in the destination Texture.");
        }
        uint effectiveDstArrayLayers = (destination.Usage & TextureUsage.Cubemap) != 0
            ? destination.ArrayLayers * 6
            : destination.ArrayLayers;
        ...
        {
            throw new Exception($"An invalid mip range was given for the destination Texture.");
        }
#endif
        CopyTextureCore(
            source,
    ...
    private void ValidateIndexBuffer(uint indexCount)
    {
        ...
        {
            throw new Exception($"An index buffer must be bound before {nameof(CommandList)}.{nameof(DrawIndexed)} can be called.");
        }
    ...
    private void PreDrawValidation()
    {
        ...
        if (_graphicsPipeline == null)
        {
            throw new Exception($"A graphics {nameof(Pipeline)} must be set in order to issue draw commands.");
        }
        if (_framebuffer == null)
        {
            throw new Exception($"A {nameof(Framebuffer)} must be set in order to issue draw commands.");
        }
        if (!_graphicsPipeline.GraphicsOutputDescription.Equals(_framebuffer.OutputDescription))
        {
            throw new Exception($"The {nameof(OutputDescription)} of the current graphics {nameof(Pipeline)} is not compatible with the current {nameof(Framebuffer)}.");
        }
#endif
    }
}
